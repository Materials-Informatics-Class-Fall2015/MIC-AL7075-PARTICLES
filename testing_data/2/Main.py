...
def Nucleation():
    ...
    sl = int(round(len(elements)**(1/3.0)))
    ids = [len(elements)/2+(sl%2-1)*((sl**2)/2-sl/2)] ## works for any cubic mesh to find approximate center
    ...
    AppendCrackHistory()
    crack.updateCrackProperties(points, 0)
...
def MSC(ith_grain_to_crack):
    ...
    # target_distance = .003    ## mesoscopic propagation distance
    ## create list of perimeter elements
    perimeter_elements = {}
    
    for point in points:
        if(point.element not in perimeter_elements and point.element not in eliminated_elements):
            temp = CC.PerimeterElement()
            temp.id = point.element
            temp.points.add(point.id)
            perimeter_elements[point.element] = temp
    ...
    ## TODO will only work in cubic meshes, need a way to expand this for non-uniform meshes
    ...
    while prop_cycles > 0 :
        ...
        for p in points:
            if(p.element!=-1):
                ...
                p.direction
                logging.debug("element %d" % p.element)
                logging.debug("plane %s" % str(plane))
                logging.debug("dir %s" % str(p.direction))
                logging.debug("crack %s" % str(crack.plane))
        ...
        for e in newPerim:
            ...
            if(temp_prop > max_prop):
                max_prop = temp_prop
        AppendCrackHistory()
        crack.updateCrackProperties(points, crack.cycles)
...
def ChangePointElements(base_id, el_points):
    ...
    global elements, perimeter_elements, points, eliminated_elements
    ...
    for i in el_points:
        point = points[i]
        within_element = np.max(np.absolute(point.point-extend_element.centroid)) < (extend_element.volume**(1.0/3))/2*(1 + tolerance)
    ...
    for n in elements[base_id].neighbors:
        ...
        for i in temp_points:
            point = points[i]
            within_element = np.max(np.absolute(point.point-extend_element.centroid)) < (extend_element.volume**(1.0/3))/2*(1 + tolerance)
            if(within_element and n not in eliminated_elements):
                point.element = n
                ...
                temp_temp_points.append(point.id)
                if(within_element and n in eliminated_elements):
                    logging.debug("attempted to add point %d to eliminated element %d" % (point.id, n))
        temp_points = temp_temp_points
...
def UpdatePropagation(el, SII=True):
    ...
    if(SII):
        ...
        if(max_FIP_1<=0 and max_FIP_2<=0):
            p1 = crack.plane
            FIP = 0
            logging.debug("element %d had 0 FIP" % (el.id))
        elif((np.linalg.norm(v1)-np.linalg.norm(v2))>eps):
            ...
            logging.debug(dir_proj1)
            logging.debug(dir_proj2)
            logging.debug(max_FIP_1)
            logging.debug(max_FIP_2)
