...
#ifdef WRITEALL
        if ((kstep.gt.n_steps_per_cycle).and.(mod(kstep,n_steps_per_cycle)
     &    .eq.0).and.pre_step) then
                   isMax_E_p_in_num = .true.
                   isMin_E_p_in_num = .false.
      else if ((kstep.gt.n_steps_per_cycle).and.(mod(kstep,n_steps_per_cycle)
...
c-----------------------------------------------------
c Record the appropriate half of the plastic strain tensor for the element
c-----------------------------------------------------	  
